...
{
	QSPI_CommandTypeDef cmd;
	uint8_t dat[2] = {flash1_dat, flash2_dat};
...
{
	uint8_t flash1_dat = 0, flash2_dat = 0;
	do
	{
		W25QXX_ReadSR(1, &flash1_dat, &flash2_dat);
	}while(((flash1_dat & 0x01) == 0x01) && ((flash2_dat & 0x01) == 0x01));
//	QSPI_CommandTypeDef cmd;
...
static void W25QXX_EnterQPIMode(void)
{
	...
	W25QXX_ReadSR(2, &flash1_dat, &flash2_dat);
    if (((flash1_dat & 0x02) == 0x00) || ((flash2_dat & 0x02) == 0x00))
    {
        W25QXX_WriteEnable();
        flash1_dat |= 0x02;
		flash2_dat |= 0x02;
        W25QXX_WriteSR(2, flash1_dat, flash2_dat);
    }
...
{
	uint8_t flash1_dat  = 0, flash2_dat = 0;
	W25QXX_ReadSR(3, &flash1_dat, &flash2_dat);
	if(((flash1_dat & 0x01) == 0x01) && ((flash2_dat & 0x01) == 0x01))	
		return;
